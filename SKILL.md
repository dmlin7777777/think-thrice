...
**合体使用场景**：新 idea 经 build-forward 分类为 B 类且用户确认"现在做"→ 交接给 incremental-implementation 执行分步实现。
...
### 核心三技能闭环（build-forward → ideas-inbox → vibecoding-workflow）

```
新想法到达 → build-forward（分类+评估）→ B/C 类入 ideas-inbox（归档+冷却）
           → 确认执行 → vibecoding-workflow（执行纪律：如何安全地改）
```

| Skill | 角色 | 交接点 |
|-------|------|--------|
| **ideas-inbox** | B/C 类处置依赖 ideas-inbox 作为归档机制 | build-forward 分类后，B/C 类移交 inbox |
| **vibecoding-workflow** | 管执行纪律（如何改） | build-forward 选定接入模式后，交接给 vibecoding-workflow |
| **incremental-implementation** | 分步执行（已知需求稳着来） | 接入模式选定后，具体实现可衔接；处理"已知需求"的"如何实现" |
| **superpowers 体系** | brainstorming（需求前）→ build-forward（新想法到达）→ executing-plans（执行+检查点） | build-forward 填补了 superpowers 中"新想法处理"的空白 |

### 互补叙事

- **brainstorming / grill-me**：负责开发**前**的需求澄清（"你到底要什么？"）
- **build-forward**：负责开发**中**的新想法接入（"这个新想法怎么放进去？"）
